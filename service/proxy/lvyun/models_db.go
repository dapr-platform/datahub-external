...
type LvyunReservation struct {
	...
	ArrDate        time.Time      `gorm:"index" json:"arr_date"`                  // 入住日期
	DepDate        time.Time      `gorm:"index" json:"dep_date"`                  // 离店日期
	...
	CreateDatetime time.Time      `json:"create_datetime"`                        // 预定时间
	...
	SyncTime       time.Time      `gorm:"index" json:"sync_time"`                 // 同步时间
...
type LvyunCheckout struct {
	...
	GuestName string         `gorm:"size:200" json:"guest_name"`             // 姓名
	ArrDep    time.Time      `json:"arr_dep"`                                // 到达日期
	DepDate   time.Time      `json:"dep_date"`                               // 离开日期
	Mobile    string         `gorm:"size:50" json:"mobile"`                  // 手机号
	SyncTime  time.Time      `gorm:"index" json:"sync_time"`                 // 同步时间
	UniqueKey string         `gorm:"size:200;uniqueIndex" json:"unique_key"` // 唯一键
}
